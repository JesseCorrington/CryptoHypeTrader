...
# We need a function that takes the predictions from multiple models and plots them together
#def plot_predictions(models, option = 'all'):
#
#            def plt(models, ID, symbol):
#
#                data = []
#                for model in models:
#                    X = model.Xtest[model.test_meta.coin_id == ID]
#                    actual_price = model.Ytest[model.test_meta.coin_id == ID]
#                    predicted_price = model.combiner_predict(X)
#                    dates = model.test_meta.date[model.test_meta.coin_id == ID]
#                    name = model.Ytest.name
#
#                    Actual = Scatter(
#                        x = dates,
#                        y = actual_price,
...
#                        x = dates,
#                        y = predicted_price,
#                        name = 'Predicted closing Price: %s' %(name)
#                        )
#
#
#                else:
...
#
#            if option == 'all':
#                for ID in pd.unique(model.test_meta.coin_id):
#                    symbol = model.test_meta.symbol[model.test_meta.coin_id == ID].iloc[0]
#                    plt(ID, symbol, filename, auto_open, custom_tag)
#
#            else:
#                ID = option
#                symbol = model.test_meta.symbol[model.test_meta.coin_id == ID].iloc[0]
